document.addEventListener('DOMContentLoaded', () => {
    ...
    const buildBanner = () => {
        ...
        container.innerHTML = `
            <div id="cck-banner-backdrop" class="cck-banner-wrap"></div>
            <div id="cck-banner" class="cck-banner">
                ...
                <div class="cck-settings">
                    <div class="cck-settings-header">
                        <h3 class="cck-settings-title">${texts.personalize}</h3>
                    ...
                    <div class="cck-actions">
                        <button id="cck-save-btn" class="cck-btn cck-btn-primary">${texts.savePreferences}</button>
                    </div>
                </div>
            </div>
        `;
    ...
    const saveConsent = (action, details) => {
        ...
        fetch(data.ajax_url, { method: 'POST', body: formData });
    };
    ...
    const addEventListeners = () => {
        document.getElementById('cck-accept-btn').addEventListener('click', () => saveConsent('accept_all', { preferences: true, analytics: true, marketing: true }));
        document.getElementById('cck-reject-btn').addEventListener('click', () => saveConsent('reject_all', { preferences: false, analytics: false, marketing: false }));
        ...
        document.querySelectorAll('.cck-switch input').forEach(input => {
            input.addEventListener('change', (e) => {
                consent[e.target.dataset.consent] = e.target.checked;
            });
        });

        document.getElementById('cck-save-btn').addEventListener('click', () => saveConsent('custom_selection', consent));
    };
