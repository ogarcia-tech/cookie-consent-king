    ...
    $translations = [
        ...
        wp_localize_script(
            ...
            [
                ...
                'reject_bg_color'   => $banner_styles['reject_bg_color'] ?? '#000000',
                'reject_text_color' => $banner_styles['reject_text_color'] ?? '#ffffff',
            ]
        );
...
function cck_settings_init() {
    ...
    add_settings_field(
        'cck_accept_button_colors',
        __('Accept/Reject Buttons', 'cookie-consent-king'),
        'cck_field_accept_button_colors',
        'cck-banner-styles',
        'cck_banner_styles_section'
    );
...
function cck_field_banner_bg_color() {
    ...
    echo '<input type="color" name="cck_banner_styles_options[bg_color]" value="' . esc_attr($value) . '" />';
}

function cck_field_banner_text_color() {
    $options = get_option('cck_banner_styles_options', []);
    $value   = $options['text_color'] ?? '#000000';
    echo '<input type="color" name="cck_banner_styles_options[text_color]" value="' . esc_attr($value) . '" />';
}

// ** NUEVA FUNCIÓN PARA LOS COLORES DE BOTONES **
function cck_field_accept_button_colors() {
    ...
    echo '<p>' . __('These colors apply to both "Accept All" and "Reject All" buttons.', 'cookie-consent-king') . '</p>';
...
function cck_field_default_message() {
    ...
    echo '<textarea name="cck_default_texts_options[message]" rows="5" cols="50" class="large-text">' . esc_textarea($value) . '</textarea>';
}
...
function cck_field_cookie_list() {
    ...
    $value   = $options['list'] ?? '';
    echo '<textarea name="cck_cookie_list_options[list]" rows="5" cols="50" class="large-text">' . esc_textarea($value) . '</textarea>';
}
...
function cck_render_dashboard() {
    ...
    <div class="wrap">
        ...
        <div id="cck-dashboard-metrics" style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px; margin-bottom: 30px;">
            <div class="postbox" style="padding: 15px; flex: 1; min-width: 200px; box-sizing: border-box;">
                <h2 class="hndle"><?php esc_html_e( 'Total Interactions', 'cookie-consent-king' ); ?></h2>
                <div class="inside" style="font-size: 2em; font-weight: bold;"><?php echo esc_html( number_format( (int) $total_logs ) ); ?></div>
            </div>
            <div class="postbox" style="padding: 15px; flex: 1; min-width: 200px; box-sizing: border-box;">
        ...
        <div id="cck-charts-and-logs" style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px;">
            <div id="cck-chart-container" style="flex: 1 1 500px;">
                ...
                <div class="postbox"><div class="inside"><canvas id="cck-consent-chart" style="max-height: 300px;"></canvas></div></div>
            </div>
        ...
        <style>
            .cck-action-badge { display: inline-block; padding: 4px 8px; border-radius: 4px; color: #fff; font-size: 0.9em; text-transform: capitalize; }
...
function cck_render_banner_styles() {
    if (
        ...
        $options = [
            ...
            'accept_bg_color' => sanitize_hex_color($input['accept_bg_color'] ?? '#000000'),
            'accept_text_color' => sanitize_hex_color($input['accept_text_color'] ?? '#ffffff'),
            'position'   => $position,
        ];
        update_option('cck_banner_styles_options', $options);
        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Settings saved.', 'cookie-consent-king') . '</p></div>';
    }
...
function cck_render_cookie_list() {
    ...
    echo '<form method="post">';
    settings_fields('cck_cookie_list_group');
...
function cck_render_translations() {
    ...
    if (
        ...
        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Translations saved.', 'cookie-consent-king') . '</p></div>';
    }
    ...
    if (
        ...
        cck_generate_mo($languages_dir);
        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('File imported.', 'cookie-consent-king') . '</p></div>';
    }
    ...
    if (
        ...
        cck_generate_mo($languages_dir);
        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('.mo files regenerated.', 'cookie-consent-king') . '</p></div>';
    }
    ...
    foreach ($strings as $id => $vals) {
        ...
        foreach ($locales as $locale) {
            ...
            echo '<td><textarea name="trans_' . esc_attr($locale) . '[' . esc_attr($id) . ']" rows="1" style="width: 100%;">' . $val . '</textarea></td>';
        }
        echo '</tr>';
    }
    echo '</tbody></table>';
