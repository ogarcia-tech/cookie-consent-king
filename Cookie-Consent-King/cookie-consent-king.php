...
// *** NUEVO: Hook para registrar textos para traducción al guardarlos ***
add_action('update_option_cck_default_texts_options', 'cck_register_custom_strings_for_translation', 10, 2);
...
function cck_enqueue_frontend_assets() {
    ...
    // *** MEJORA: Obtener textos traducidos si existen ***
    $banner_title = $texts_opts['title'] ?? __('Gestión de Cookies', 'cookie-consent-king');
    $banner_message = $texts_opts['message'] ?? __('Utilizamos cookies para mejorar tu experiencia de navegación.', 'cookie-consent-king');

    // Polylang
    if (function_exists('pll__')) {
        $banner_title = pll__($banner_title);
        $banner_message = pll__($banner_message);
    }
    // WPML
    if (function_exists('icl_object_id')) {
        $banner_title = apply_filters('wpml_translate_string', $banner_title, 'cck-banner-title', ['plugin' => 'cookie-consent-king']);
        $banner_message = apply_filters('wpml_translate_string', $banner_message, 'cck-banner-message', ['plugin' => 'cookie-consent-king']);
    }
    ...
    $dynamic_css = "
        ...
        #cck-banner {
            ...
            width: calc(100% - 40px); max-width: 900px; background-color: var(--cck-bg-color);
            color: var(--cck-text-color); border: 1px solid #e0e0e0; border-radius: 8px;
            padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); z-index: 9999; display: none;
            font-family: sans-serif; box-sizing: border-box;
        }
        #cck-banner-content { display: flex; align-items: center; justify-content: space-between; gap: 20px; flex-wrap: wrap; }
        ...
        .cck-btn { padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; font-weight: bold; transition: opacity 0.2s; }
        .cck-btn:hover { opacity: 0.85; }
        .cck-btn-primary { background-color: var(--cck-btn-bg-color); color: var(--cck-btn-text-color); }
    ";
    ...
    <div id="cck-banner">
        <div id="cck-banner-content">
            <div id="cck-banner-text">
                <h3>' . esc_html($banner_title) . '</h3>
                <p>' . esc_html($banner_message) . '</p>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const handleConsent = (action) => {
            ...
            document.cookie = 'cookie_consent=' + action + '; expires=' + date.toUTCString() + '; path=/; SameSite=Lax';
        ...
        document.getElementById('cck-btn-accept').addEventListener('click', () => handleConsent('accept_all'));
        document.getElementById('cck-btn-reject').addEventListener('click', () => handleConsent('reject_all'));
    });
    ";
    wp_add_inline_script('jquery-core', $frontend_js);
}

// --- LÓGICA DEL PANEL DE ADMINISTRACIÓN (sin cambios) ---
...
function cck_settings_init() {
    ...
    add_settings_field('cck_privacy_url', __('Privacy Policy URL', 'cookie-consent-king'), 'cck_field_text_input', 'cck-basic-configuration', 'cck_basic-configuration_section', ['name' => 'privacy_url', 'type' => 'url']);
}
...
function cck_field_button_colors() {
    ...
    echo '<label>' . __('Background', 'cookie-consent-king') . ': </label><input type="color" name="cck_banner_styles_options[accept_bg_color]" value="' . esc_attr($bg_color) . '" />';
    echo '<label style="margin-left: 15px;">' . __('Text', 'cookie-consent-king') . ': </label><input type="color" name="cck_banner_styles_options[accept_text_color]" value="' . esc_attr($text_color) . '" />';
}
...
function cck_field_text_input($args) {
    $group = ($args['name'] === 'title') ? 'cck_default_texts_options' : 'cck_basic_configuration_options';
    $options = get_option($group, []);
    $value = $options[$args['name']] ?? '';
    echo '<input type="' . esc_attr($args['type'] ?? 'text') . '" name="' . $group . '[' . esc_attr($args['name']) . ']" value="' . esc_attr($value) . '" class="regular-text" />';
}
...
function cck_render_dashboard() {
    ...
    <div class="wrap">
        ...
        <script>new Chart(document.getElementById('cck-chart'),{type:'bar',data:{labels:<?php echo json_encode($labels);?>,datasets:[{label:'<?php esc_html_e('Actions', 'cookie-consent-king');?>',data:<?php echo json_encode($data);?>}]}});</script>
    </div>
    <?php
}

function cck_render_banner_styles() { echo '<div class="wrap"><h1>' . esc_html__('Banner Styles', 'cookie-consent-king') . '</h1><form method="post" action="options.php">'; settings_fields('cck_banner_styles_group'); do_settings_sections('cck-banner-styles'); submit_button(); echo '</form></div>'; }
function cck_render_default_texts() { echo '<div class="wrap"><h1>' . esc_html__('Default Texts', 'cookie-consent-king') . '</h1><form method="post" action="options.php">'; settings_fields('cck_default_texts_group'); do_settings_sections('cck-default-texts'); submit_button(); echo '</form></div>'; }
function cck_render_basic_configuration() { echo '<div class="wrap"><h1>' . esc_html__('Basic Configuration', 'cookie-consent-king') . '</h1><form method="post" action="options.php">'; settings_fields('cck_basic_configuration_group'); do_settings_sections('cck-basic-configuration'); submit_button(); echo '</form></div>'; }

// --- FUNCIONES AUXILIARES ---

// *** NUEVA FUNCIÓN PARA REGISTRAR CADENAS ***
function cck_register_custom_strings_for_translation($old_value, $new_value) {
    if (isset($new_value['title'])) {
        $title = $new_value['title'];
        if (function_exists('pll_register_string')) {
            pll_register_string('cck-banner-title', $title, 'Cookie Consent King');
        }
        if (function_exists('icl_object_id')) {
            do_action('wpml_register_single_string', 'cookie-consent-king', 'cck-banner-title', $title);
        }
    }
    if (isset($new_value['message'])) {
        $message = $new_value['message'];
        if (function_exists('pll_register_string')) {
            pll_register_string('cck-banner-message', $message, 'Cookie Consent King');
        }
        if (function_exists('icl_object_id')) {
            do_action('wpml_register_single_string', 'cookie-consent-king', 'cck-banner-message', $message);
        }
    }
}

function cck_load_textdomain() { load_plugin_textdomain('cookie-consent-king', false, dirname(plugin_basename(__FILE__)) . '/languages'); }
function cck_activate() { global $wpdb; $table_name = $wpdb->prefix . 'cck_consent_logs'; require_once ABSPATH . 'wp-admin/includes/upgrade.php'; dbDelta("CREATE TABLE $table_name (id mediumint(9) NOT NULL AUTO_INCREMENT, action varchar(50) NOT NULL, ip varchar(100) DEFAULT '' NOT NULL, country varchar(100) DEFAULT '' NOT NULL, created_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY  (id)) " . $wpdb->get_charset_collate() . ";"); }
function cck_get_user_ip() { if(!empty($_SERVER['HTTP_CLIENT_IP'])) return sanitize_text_field(wp_unslash($_SERVER['HTTP_CLIENT_IP'])); if(!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) return sanitize_text_field(trim(explode(',', wp_unslash($_SERVER['HTTP_X_FORWARDED_FOR']))[0])); return sanitize_text_field($_SERVER['REMOTE_ADDR'] ?? ''); }
function cck_get_country_from_ip($ip) { if (empty($ip)) return ''; $response = wp_remote_get('https://ipapi.co/' . $ip . '/country/'); if (is_wp_error($response)) return ''; return sanitize_text_field(wp_remote_retrieve_body($response)); }
function cck_log_consent() { $action = sanitize_text_field($_POST['consent_action'] ?? ''); if (!$action) wp_send_json_error('Missing action'); global $wpdb; $ip = cck_get_user_ip(); $wpdb->insert($wpdb->prefix . 'cck_consent_logs', ['action' => $action, 'ip' => $ip, 'country' => cck_get_country_from_ip($ip), 'created_at' => current_time('mysql')]); wp_send_json_success(); }
function cck_export_logs() { if (!current_user_can('manage_options')) wp_die(__('Unauthorized', 'cookie-consent-king')); global $wpdb; $logs = $wpdb->get_results("SELECT created_at, action, ip, country FROM " . $wpdb->prefix . "cck_consent_logs ORDER BY id DESC", ARRAY_A); header('Content-Type: text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename="cck_consent_logs.csv"'); $output = fopen('php://output', 'w'); fputcsv($output, ['Date', 'Action', 'IP', 'Country']); if ($logs) { foreach ($logs as $log) { fputcsv($output, $log); } } fclose($output); exit; }